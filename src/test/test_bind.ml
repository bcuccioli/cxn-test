...
let test_bind_success _ =
  ...
  let bound =
    [
      { cmd = Ping; src = "x"; dst = "y"; res = Accept; usr = Some "u" };
      { cmd = Ssh; src = "x"; dst = "z"; res = Reject; usr = None };
    ]
  in
...
let test_bind_sequential_success _ =
  ...
  let bound =
    [
      { cmd = Ping; src = "x"; dst = "y"; res = Accept; usr = Some "u" };
      { cmd = Ping; src = "c"; dst = "d"; res = Reject; usr = None };
      { cmd = Ping; src = "z"; dst = "d"; res = Reject; usr = None };
    ]
  in
  assert_equal bound (bind stmts)
...
let test_bind_shadow_fail _ =
  ...
  in
  assert_raises (ShadowError ("a", "z", "x")) (fun () -> bind stmts)
